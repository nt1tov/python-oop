...
pygame.display.set_caption("Knight Wars 2020")
...
def create_game(sprite_size, is_new):
    ...
    if is_new:
        hero = Objects.Hero(base_stats, Service.create_sprite(os.path.join("texture", "hero-2.png"), sprite_size))
        engine = Logic.GameEngine()
        engine.add_hero(hero)
        Service.service_init(sprite_size)
        engine = Service.reload_game(engine)
 
        drawer = SE.GameSurface((640, 480), pygame.SRCALPHA, (0, 480), 
                SE.ProgressBar((640, 120), (640, 0),
                SE.InfoWindow((160, 600), (0, 0),
                SE.HelpWindow2((800, 600), pygame.SRCALPHA, (0, 0),
                SE.StartWindow((800, 600), pygame.SRCALPHA, (0,0),  
                SE.ScreenHandle((0, 0)
                                        ))))))
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key ==  pygame.K_SPACE:
                        engine.show_start = not engine.show_start
